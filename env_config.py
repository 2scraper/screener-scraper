...
Family core. Two reasons this is hand-rolled rather than
`python-dotenv`:
...
1. **No new dependency.** Every engine here already refuses to be a heavy
   ...
   *is* installed it gets used instead, so a project that already depends on
   it keeps its own behaviour.
2. **A secret in argv is visible to anything that can run `ps`.** Passing
   `--twocaptcha-key 1234...` leaks the key to every other process on the
...
That order matters in both directions: a `.env` must never silently override
something the caller typed, and an already-exported variable (CI secret,
`direnv`, a shell profile) must never be clobbered by a file someone forgot
to delete.

Nothing here is required. With no `.env` and no environment variables the
scripts behave exactly as before.

Run `python3 env_config.py` to see what would be picked up, WITHOUT printing
...
ENV_KEYS = {
    ...
    "SCREENER_CDP_ENDPOINT": "cdp_endpoint",
...
def is_placeholder(value):
    """True when `value` is absent, empty, or an uncompleted example.

    this family's template, measured across four repos at once: the placeholder check
    used to be a literal set, and the two credentialled URLs 2Captcha
    documents are written with braces in the VENDOR'S OWN DOCS —

        ws://{login}-zone-scraping_browser-...:{password}@cb.2captcha.com:9222
        http://{user}:{password}@xx.proxy.2captcha.com:2334

    — so `cp .env.example .env` followed by a run connected with the literal
    string "{login}-zone-…" as its username and got a 401: the confusing auth
    error a long way from its cause that this module exists to prevent. Any
    `{…}` left in a value therefore reads as unset, generically.

    An EMPTY value is unset and nothing more: it is what an unset CI secret
    expands to, and warning about it would print a warning about nothing on
    every canary run that has no proxy configured.
    """
    if value is None:
        return True
...
def documented_keys(path=None):
    """Every variable name written in `.env.example`.

    Used by the offline suite to assert that the example and ENV_KEYS are
    equal in BOTH directions: an undocumented variable is bad, and a
    documented-but-unread one is worse, because it looks configurable.
    """
    if path is None:
...
def unknown_keys(path=None):
    ...
    Usually a typo — `TWO_CAPTCHA_KEY`, `SCREENER_CDP` — which otherwise fails
...
def env_value(name):
    """Read one recognised variable, treating example placeholders as unset.

    The warning names the VARIABLE and never quotes the value: the value can
    itself be a credentialled URL.
    """
    ...
    if is_placeholder(value):
        if value.strip():
            logger.warning(
                "%s is still set to the placeholder from .env.example — "
