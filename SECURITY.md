...
**"screener-scraper security"** in the subject — otherwise it lands in a queue
...
- **Getting past screener.in's registration wall.** This scraper reads what
  the site serves to an anonymous visitor and reports the wall as blocked
  when it meets it; it does not log in and does not try to. Anything about
  how the site decides whom to show the wall is not a vulnerability here.
- **The scraper stopped working.** screener.in changing its markup is
  expected — file it as a normal issue, there is a template for exactly that.
...
Two places leak credentials that people do not expect, because unlike our own
log lines they are **not** masked:

- **raw HTML dumps** (`--dump-html`, and the automatic dump on a blocked or
  unparsed page) — on this site these carry the session's anonymous
  `csrfmiddlewaretoken`, and a page fetched through a browser that is logged
  in would carry that session
- **your shell history**, if you passed a key on the command line

The Scraper API's `x-debug` response header used to be a third: it echoes the
task back, credentialled `cdpurl` included, and sibling repos logged it
verbatim. This repo redacts it before logging, and the offline suite pins that
with a credentialled fixture.
